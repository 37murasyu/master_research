...
class GaugeDisplay:
    ...
    def _init_figure(self):
        ...
        if os.path.exists(self.image_path):
            ...
            # 画像が存在しない場合のフォールバック（白背景）
            img = np.ones((100, 100, 3), dtype=float)
        ...
        for c, label in ((g["center"], g["label"]) for g in self.config["gauges"]):
            # 灰色ベースとグラデーションパッチ
            for j in range(180, 120, -2):
                self.ax.add_patch(Wedge(c, self.radius, j, j + 2, facecolor="lightgray", lw=0))
            for j in range(120, -1, -2):
                self.ax.add_patch(Wedge(c, self.radius, j, j + 2, facecolor=cmap(j / 120), lw=0))
            # 針とラベル
            ...
            label_name = self.ax.text(c[0], c[1] + 2.1, label, ha="center", fontsize=10, color="black")
            outline = Wedge(c, self.radius, 0, 180, facecolor="none", edgecolor="red", lw=2)
            outline.set_visible(False)
            self.ax.add_patch(outline)
            self.gauges.append({"center": c, "needle": needle, "label_val": label_val, "label_name": label_name, "outline": outline})
        # 軸範囲設定
    ...
    def get_angles(self) -> list[float]:
        ...
        for pk in self.part_keys:
            ...
            imp = self.current_impulses.get(pk, 0.0)
            sigma_safe = sigma if abs(sigma) > 1e-9 else 1.0
            angle = 60 * (imp / sigma_safe) + 120 - 60 * (mu / sigma_safe)
            # 角度の安全域（0〜180）にクリップ
            angles.append(float(np.clip(angle, 0, 180)))
        return angles
