...
POSE_FILES = [
    ...
    "output_data/filtered_pose_lpf/3_1stereo_pose_scaled_with2d_lpf.csv",
...
OUT_DIR = Path("fft_plots/pose3d_interactive")
...
def plot_file(csv_path: str) -> Path:
    ...
    fig.update_layout(
        title=f"3D Pose (frame {frame_idx})",
        scene=dict(xaxis_title="X", yaxis_title="Y", zaxis_title="Z")
    )

    out_path = OUT_DIR / f"{Path(csv_path).stem}_frame{frame_idx}.html"
