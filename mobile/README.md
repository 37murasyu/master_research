...
### 計測中の速さの表示と 30fps 固定（2026-09-23）

実機の Pixel 7a は PC に 10〜15 Hz しか点が届かなかった。どこで減っているかを見分けるため、計測中の画面に
段階ごとの 1 秒あたりの数を出す（`pose/StageRates.kt`）。

```
毎秒 カメラ 15.0 → 解析 15.0 → 推論 14.8 → 人 14.3 → 送信 14.3（カメラが遅い: 照明を明るくするか 30fps 固定にする）
```

- カメラ: カメラが撮った枚数（Camera2 の撮影完了。解析に渡らなかった分も数える）。24 未満なら「カメラが遅い」と出す
- 解析 < カメラ: 解析が追いつかず捨てている／推論 < 解析: 推論中に来たフレームを MediaPipe が捨てている／
  人 < 推論: 検出が外れている／送信 < 人: 送信の待ち行列が溢れている。前の段より 2 割以上減った最初の段を示す

画面右下の「30fps固定」（既定はオン）は、自動露出の目標フレームレートを 30 fps に固定する（`CameraSetup` の
`CONTROL_AE_TARGET_FPS_RANGE`）。暗い室内では自動露出が露光を 1/15 s ほどに延ばし、カメラ自体が 15 fps に落ちるため。
固定すると映像は暗くなるので、人の検出が落ちるなら照明を足す。計測中に切り替えると、その場でカメラを開き直す。
当日の手順は `docs/hybrid_field_run.md`。
