...
## 引き継ぎ追加対応（2026-09-23、未コミット）

（追記: この節の変更は同日のうちに d3ce0ee・13a34b1・9a72f2c・cc61163・6fac946 としてコミットした）

- 開始時点: `murayama/fix-left-right-dynamics`、HEAD `cd94230`。既存の未追跡 `HANDOFF_TODO.md` を引き継ぎ、更新。
- 基準テスト: 482 passed / 1 skipped。
- §6-5・§4-4・体幹係数の回帰テストを先に追加し、16件の失敗を確認してから修正。
  範囲外は `cycle_index=-1`、旧トルクCLIは現行の辞書APIへ接続。
  旧CLIの手首・支持モデルは互換用として保持し、論文再計算には使わない旨を明記。
- 真偽値設定は引き継ぎの7か所に `E_FC_ADAPTIVE_ON` を加えた8か所を統一。
  設定スキーマを生成し直すと、先行コミットで廃止済みの `GRAVITY_DETECT_FRAMES` も除去された。
- ネイティブ描画の追跡ファイルと、残存参照のあった専用手動テストを削除。
- 被験者4のノイズ評価のCLIテスト2件が失敗することを確認後、固定除外をやめ、明示引数で除外可能にした。
  本番データの範囲・集計対象は変更しておらず、8名の再集計は未実施。
- スマホ1台＋PCカメラ用送信器: テスト3件の失敗を確認後、PC単調時計の撮影時刻と元画像サイズを送る実装を追加。
  推論は専用スレッドで動かし、模擬スマホとの同期、切断・未検出、終了時解放を確認。
  実機とGUI／保存の統合は未完了。手順と制約は `docs/phone_pc_capture.md`。
- 上腕質量比は初期コミットと参考URLまで追跡。参考コードは0.027で、現行0.0227の根拠は未確定。
- ユーザー回答: Windowsのデータは収集中。カメラの水平設置は不明、重力は現状維持。
- コミット・pushは行わない。判断待ちの分母、1RM列、前処理、集計条件、本文は保留。


検証結果（追加対応後）:

- `.venv/bin/python -m pytest -q tests`: **506 passed / 1 skipped**、既存の非推奨警告2件。
- `.venv/bin/python tools/verify_dynamics_recheck.py`: 全節OK。
- `.venv/bin/python -m py_compile master_research_code.py`: 成功。
- 変更した補助モジュール・追加テストの `uvx --quiet pyflakes`: 指摘なし。
  `master_research_code.py` はHEADにもある既存の23件のみで、新規指摘なし。
- `git diff --check`: 成功。


## Windows転送受領後の照合（2026-09-23）

- ZIPを `~/Downloads/transfer_20260923_review/extracted/` に展開。CRC成功、一覧361件のパス・サイズ一致。
  提示SHA-256とは異なる。調査開始時は照合待ちだったが、送信側の最終SHAは確認できないとのユーザー回答を受け、
  再作成・再送を求めず受領版を解析の基準にする。送信側との同一性は未確認と記録し、待ち条件から外した。
- GCVSPL系の論文0.99/0.88を再現。14件の分母は旧ゲージ式と一致し、同梱説明のLPF式との混同を訂正。
  LPF系のバグをGCVSPL系へ直接当てはめないようKNOWN_ISSUESの従来の主張を限定した。
- 旧Git版2fb8ff6でtorque_wristbaseの8試技を再現し、倍率0.01を確認。
- 受領outer列で同じ7試技を現行モデルにより別フォルダへ仮再計算。前回proxy条件も再計算して比較。
- 手の点付き処理済み姿勢は無し。動画18本はあるが、再推定の前に校正・単位等の確認が残る。
- 隔離7件はWindows報告あり、移動後証跡は無し。実機試験、元シート列・手首分母・集計条件の判断は引き続き保留。
- 詳細: `docs/windows-transfer-audit-2026-09-23.md`。今回のリポジトリ変更は文書のみ、コミット・pushなし。
